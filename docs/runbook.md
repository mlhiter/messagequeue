...
same-name shell asset files, verify `index.html`, `app.js`, `styles.css`, and
`config.js` return `Cache-Control: no-store`, and verify the built `index.html`
references `app.js`, `styles.css`, and `config.js` with content-hash query
strings after frontend rollouts.
