...
| 连接信息 | `frontend/app.js` | `messagequeue.detail.connections` | panel | Bootstrap 和环境变量 | none | 环境变量复制时是 | 是 | API | 凭据泄露 |
...
| 环境变量 | `frontend/app.js` | `messagequeue.detail.connections` | config | 默认遮蔽密码和 CA，复制时按需获取真实凭据并写入剪贴板 | none | 复制时是 | 是 | API | 凭据泄露 |
...
- 普通 `client-config` 只能包含 secret-free 元数据；环境变量复制流程可以返回密码和 CA，但不得把真实值写入语义标签、DOM `data-copy`、日志、URL、指标标签或 raw query。
